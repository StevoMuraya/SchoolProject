...
if (
    ...
    $qr_code      = filter_var($_POST['qr_code'], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH);
...
function fetch_class_details($student_id, $qr_code, $db_handle, $result)
{
    ...
    if (mysqli_num_rows($response) > 0) {
        ...
        if (check_student_class($student_id, $row['class_id'], $db_handle) == 1) {
            $result['success'] = "1";
            $result['message'] = "Student in class already";
            ...
            if (add_student_to_class($student_id, $row['class_id'], $db_handle) == 1) {
                $result['success'] = "1";
                $result['message'] = "Student added to class";
                if (check_student_scan_dublicate($student_id, $qr_code, $db_handle) == 1) {
                    $result['success'] = "3";
                    $result['message'] = "Student already signed";
                } else {
                ...
                $result['success'] = "5";
                $result['message'] = "Failed to add student to class";
            }
        }
